...
Name:            xorg-x11-drv-nvidia-bumblebee
Epoch:           1
Version:         280.13
Release:         1%{?dist}
...
install -m 0755 -d $RPM_BUILD_ROOT%{nvidialibdir}/vdpau/
...
install -m 0755 -d $RPM_BUILD_ROOT%{nvidialibdir}/xorg/modules/extensions/nvidia/
install -m 0755 -d $RPM_BUILD_ROOT%{nvidialibdir}/xorg/modules/drivers/
rm -f $RPM_BUILD_ROOT%{nvidialibdir}/lib{nvidia-wfb,glx,vdpau*}.so.%{version}

# Finish up the special case libs
install -p -m 0755 libnvidia-wfb.so.%{version} $RPM_BUILD_ROOT%{nvidialibdir}/xorg/modules/extensions/nvidia/
install -p -m 0755 libglx.so.%{version}        $RPM_BUILD_ROOT%{nvidialibdir}/xorg/modules/extensions/nvidia/
install -p -m 0755 nvidia_drv.so               $RPM_BUILD_ROOT%{nvidialibdir}/xorg/modules/drivers/
install -p -m 0755 libvdpau*.so.%{version}     $RPM_BUILD_ROOT%{nvidialibdir}/vdpau/
...
for lib in $( find $RPM_BUILD_ROOT%{nvidialibdir} -name lib\*.%{version} ) ; do
...
%dir %{nvidialibdir}/xorg/modules/extensions/nvidia
%{nvidialibdir}/xorg/modules/drivers/nvidia_drv.so
%{nvidialibdir}/xorg/modules/extensions/nvidia/*.so*
...
%{nvidialibdir}/tls/*.so*
%exclude %{nvidialibdir}/vdpau/libvdpau.*
%{nvidialibdir}/vdpau/libvdpau_nvidia.so*
...
- Forked from xorg-x11-drv-nvidia to add experimental bumblebee support
- Remove nvidia-%{_lib}.conf to not influence intel xorg libs
- change some directorys from "_libdir" to "nvidialibdir"
- removed xorg nvidia.conf (is included in bumblebee)
